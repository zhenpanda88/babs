...
int main(int argc, char **argv) {
    ...
    if(input_command<1012)
    {
        input_command = 1012;
        ROS_WARN("WARNING: ENTERED POSITION GOES BEYOND PHYSICAL LIMITS; Setting position to 1012");
    }
    if(input_command>3036)
    {
        input_command = 3036;
        ROS_WARN("WARNING: ENTERED POSITION GOES BEYOND PHYSICAL LIMITS; Setting position to 3036");
    }
    ...
    {
        if(g_current_wobbler_angle > input_command)
        {
            command = input_command;
        }
        else if (g_current_wobbler_angle < input_command)
        {
            command = input_command;
        }
        else
    ...
    while (ros::ok()) 
    {
        ...
        input_command = (short int)input_command;
